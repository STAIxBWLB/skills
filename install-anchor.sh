#!/usr/bin/env bash
# install-anchor.sh — expose public skills through ~/.anchor/skills/installed

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")" && pwd)"
SKILLS_SRC="$REPO_ROOT/skills"
ANCHOR_HOME="${ANCHOR_HOME:-$HOME/.anchor}"
SKILLS_DST="${ANCHOR_SKILLS_DIR:-$ANCHOR_HOME/skills/installed}"
SOURCE_ID="${ANCHOR_SKILLS_SOURCE_ID:-stai-public}"
DRY_RUN=false
FORCE=false
