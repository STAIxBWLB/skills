...
- HWPX writing: use the released native `hwp` binary and its unified exported `hwp` skill tree (minimum `v0.12.1`); verify the selected absolute binary's version before generation, fill, or validation. No bundled JRE.
...
2. `$MARU_SKILLS_ENV/.venv/bin/python3` (host-injected)
3. `$HOME/.maru/env/.venv/bin/python3` (canonical fixed location)
...
venvs cannot shadow the Maru runtime. The Maru host injects `MARU_SKILLS_ENV`,
