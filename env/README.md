...
이 디렉토리(`env`)는 source scaffold 이며, `setup.sh --target ~/.anchor/env`
로 정규 런타임 `~/.anchor/env`(`.venv` + `node_modules` + `jre`)에 프로비저닝된다.
Claude Code SessionStart hook이 세션에서 이 환경을 자동 활성화한다.
...
- **skills**: `~/.anchor/skills/` Anchor-managed local install (inbox-process 등이 이 환경에 의존)
