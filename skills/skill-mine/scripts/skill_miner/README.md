...
3. **GraphBuild**: `_sys/skills/lib/build-graph.py` 의 `extract_wiki`, `build_graph`, `detect_communities` (Leiden seed=42 / Louvain fallback), `find_god_nodes`, `find_surprising_connections`, `compute_community_stats` 를 그대로 import 하여 재사용.
...
# 단계별 실행
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --stage extract --days 7
_sys/skills/skills/skill-mine/scripts/run.sh \
...
이미 설치됨 (`_sys/skills/env/pyproject.toml`):
...
```yaml
# _sys/skills/skills/skill-mine/scripts/skill_miner/vocab/actions.yaml
actions:
...
```yaml
# _sys/skills/skills/skill-mine/scripts/skill_miner/vocab/objects.yaml
objects:
  - canon: newsletter
...
- 이 파이프라인은 `vault-graph` 스킬과 동일한 `_sys/skills/lib/build-graph.py` primitives 를 사용한다. vault 그래프 분석 · skill 패턴 감지가 같은 파이프라인을 공유한다.
