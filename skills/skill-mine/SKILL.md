...
- `_sys/skills/env/.venv` 에 `graphifyy`, `networkx`, `pyyaml` 설치
...
- `_sys/skills/skills/skill-mine/scripts/skill_miner/vocab/actions.yaml` — 한·영 동사 사전
- `_sys/skills/skills/skill-mine/scripts/skill_miner/vocab/objects.yaml` — 한·영 객체 사전
- `work/project-registry.yaml` — people/orgs/keywords/acronyms (SSOT)

PII 마스킹: 사람명 → `[PERSON]`, 이메일 → `[EMAIL]`, 절대경로 → `[PATH]`.

### 3. GraphBuild (`_sys/skills/lib/build-graph.py` 재사용)
...
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --days 30

# 최근 7일만
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --days 7

# 단계별
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --stage extract
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --stage materialize
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --stage graph
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --stage analyze

# 캐시 재사용 (상위 단계 그대로, 하위만 재실행)
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --from-cache

# 임계치 조정
_sys/skills/skills/skill-mine/scripts/run.sh \
  _sys/skills/skills/skill-mine/scripts/skill_miner/run.py --min-size 2 --min-novelty 0.2
```
...
1. **승격** (수동): 적합한 후보 1건 선택 → `skill-creator`로 SKILL.md 본문 작성 → public이면 `_sys/skills/skills/<name>/`, private이면 `_sys/skills-private/skills/<name>/` 배치
...
- 스크립트: `_sys/skills/skills/skill-mine/scripts/skill_miner/` (extract/materialize/graph_build/analyze/run + vocab)
- 재사용 primitives: `_sys/skills/lib/build-graph.py`
- 관련 스킬: `vault-graph` (동일한 그래프 primitives 사용)

## 다음 단계 (선택 구현)

- `_sys/skills/skills/_drafts/YYMMDD-<slug>/SKILL.md` 템플릿 기반 자동 드래프트 (후보 → 뼈대)
