...
- **Call-time scope and unattended mode.** When the caller names the files to edit, read and edit only those. When the run is unattended, never stop to ask: a defect that needs the caller's decision goes on the `Deferred:` line and is left as is.
