...
| PDF 변환 | `./hwpx to-pdf <file.hwpx>` (기본 hwp-cli 이미지 PDF; `--engine soffice` 벡터) |
...
`to-pdf`는 기본적으로 **hwp-cli**로 변환한다 (`hwp render` → PNG → img2pdf). 레이아웃 정확,
LibreOffice 불필요. 단 결과는 **이미지 기반**(텍스트 선택 불가).

```bash
./hwpx to-pdf document.hwpx -o document.pdf            # 기본: hwp-cli (이미지)
./hwpx to-pdf document.hwpx --engine hwp --dpi 200     # hwp-cli 강제 + 해상도
./hwpx to-pdf document.hwpx --engine soffice           # LibreOffice (벡터·텍스트선택)
```

- `--engine auto`(기본): hwp-cli 우선, 미발견/실패 시 soffice 폴백.
- `--engine hwp`: hwp-cli 강제 (이미지 PDF). `render-pdf`와 동일.
- `--engine soffice`: 벡터·텍스트선택 PDF. **LibreOffice + H2Orestart 확장** 필요:
