...
2. **신규 생성 (`styled` / `create`)** — 양식이 없을 때 `hwp new`로 기본 스타일 문서를 생성. (`--preset`은 hwp-cli에서 제거돼 no-op — 하위호환용으로만 수용.)
...
- `styled --markdown <md> -o <out>` — `hwp new`로 기본 스타일 생성. `--preset gongmun|bogoseo`는 하위호환용 no-op(hwp-cli가 `--preset`을 제거 — 출력에 영향 없음).
