...
숫자로 시작하는 제목은 이중 번호 방지로 생략), 번호 목록은 네이티브 NUMBER(`1.`→`1)` 수준별,
시작 번호 인정), 불릿은 네이티브 BULLET(수준별 들여쓰기). `cat` 평문에는 마커가 안 보이는 게
정상(자동 번호)이고, markdown 출력(`to-md`)과 한글 화면에는 합성되어 표시된다.
