...
- **`<hp:t>`가 실제 텍스트 leaf**
- 런 경계를 가로지르는 텍스트는 find/replace가 실패할 수 있음 (python-hwpx의 `replace_text_in_runs`는 런 내부만 검색)
...
정확한 네임스페이스 URI는 파일마다 선언부 참조. python-hwpx는 `DEFAULT_NAMESPACES`로 제공.
...
새 이미지·섹션을 추가할 때 이 파일을 갱신하지 않으면 python-hwpx와 Hancom Office 모두 파일을 인식하지 못한다.
