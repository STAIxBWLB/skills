...
The skill already has `python-hwpx` (NC-licensed) bound through `_sys/env/.venv`. Three problems pushed us toward a parallel Java path:
...
```bash
bash _sys/env/scripts/setup-jre.sh
```
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `bundled-JRE writer 자산 누락` | `_sys/env/jre/` missing or corrupt | `bash _sys/env/scripts/setup-jre.sh` |
...
| Wrong arch ("Bad CPU type") | tidy donor was arm64 but host is x86_64 | Delete `_sys/env/jre/`, re-run `setup-jre.sh` so it falls through to Temurin download for the host arch |
