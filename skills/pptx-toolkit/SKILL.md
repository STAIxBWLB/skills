...
description: >
  ...
  (~/.maru/env/.venv 기설치). MVP는 read-only — 슬라이드 텍스트, 제목 아웃라인,
...
PowerPoint `.pptx` 파일을 read-only로 파싱하여 텍스트·제목·노트 추출, markdown 변환을 수행한다. `~/.maru/env/.venv`의 python-pptx 1.0.2 사용. `inbox-process` 추출 단계에서 발표자료 처리에 사용할 수 있다.
...
```bash
PPTX=~/.maru/skills/pptx-toolkit/pptx
...
```markdown
| `.pptx` | pptx-toolkit | `~/.maru/skills/pptx-toolkit/pptx read <file>` |
```
