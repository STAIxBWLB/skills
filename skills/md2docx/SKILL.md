...
description: >
  ...
  머리말/PAGE 푸터·〔확인필요〕 적색 강조를 프로그램적으로 제어. Anchor env에 번들된 Node
...
런타임: Anchor env(`~/.anchor/env`)에 번들된 Node + `docx` 패키지. wrapper가 env Node를
해소하고 `NODE_PATH=~/.anchor/env/node_modules`를 export하여 변환기가
...
경로 기준: `~/.anchor/skills/md2docx/md2docx`
...
```bash
bash ~/.anchor/skills/_builtin/envs/default/setup.sh --target ~/.anchor/env --verify
